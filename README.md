...
3. 파일 바이트의 SHA-256을 계산합니다. 알림은 프로젝트 ID, 수신자(등록된 agent ID), 경로, 소문자 64자리 해시를 사용합니다. 기본 등록은 `emma`/`jaemin`/`user`이며 `config/agents.json`에는 향후 어댑터용 `opencode`/`openclaw`가 비활성 상태로만 선언돼 있습니다.
...
`collaboration-audit-v1`은 어댑터가 실제로 전송·수신한 자연어 원문을 작업·실행별로 보존합니다. 저장 시 서버가 UTF-8 본문의 SHA-256을 직접 계산하고, 어댑터가 보낸 예상 해시와 대조합니다. `(project_id, event_key)`는 유일하므로 같은 실행의 재전송은 같은 본문일 때만 멱등 처리되고, 다른 본문으로 덮어쓸 수 없습니다.

- 등록: `POST /api/audit/events`
- 조회: `GET /api/audit/events?project_id=<ID>&task_id=<선택>&after=<ID>&limit=<수>`
- 화면: 선택한 작업의 기록을 기본 표시하고, **프로젝트 전체 보기**로 전체 연혁을 봅니다.
- 분리 원칙: 감사기록 테이블은 기존 메시지·작업 설명·worker context와 분리됩니다. 조회 API를 명시적으로 호출하지 않는 한 다음 AI 입력에 들어가지 않습니다.
- 범위: 외부 에이전트가 화면에 반환한 응답을 기록하며 숨은 추론 과정(chain-of-thought)을 수집하지 않습니다.
- 비밀정보: metadata를 재귀 순회하여 `password`, `passwd`, `token`, `api_key`, `secret`, `credential`, `private_key` 계열 키를 저장 거부합니다. 자연어 본문은 정확한 송수신 기록이므로 어댑터가 승인정보·자격정보를 본문에 넣지 않아야 합니다.
- 무결성: DB 트리거가 감사기록의 UPDATE·DELETE를 거부하고, 조회 때마다 저장 본문의 SHA-256을 다시 계산합니다.
...
- 자동 깨우기/상주 Hermes 브리지, 사용자 알림 push, 일반 메모 본문 보관, 취소·재시도 관리, worker 인증은 미구현입니다. 다만 제한형 Kanban–agy 어댑터의 실제 송수신 원문 감사기록은 지원합니다.
...
구성: `courier.py` 전달 상태 저장, `audit.py` 추가전용 협업 대화기록, `worker.py` 기존 작업자용 one-shot adapter, `server.py` HTTP API, `launcher.py` 서버/브라우저 실행, `store.py` 이전 저장소 및 프로젝트·원칙, `static/index.html` 우편함 UI.
